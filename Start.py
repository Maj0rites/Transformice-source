...
class Client:
    def __init__(self):
        ...
        self.langue = ""
        self.packages = ""
        self.realLangue = json.load(urllib2.urlopen("http://ipinfo.io/json"))["country"].lower()
    ...
    def connectionLost(self, args):
        ...
        if self.server.players.has_key(self.playerName):
            ...
            if self.privLevel in [5, 6, 7, 8]:
                    privbyte = {5:9, 6:7, 7:3, 8:3}[self.privLevel] # 2: arbitre, 3: modo, 7: mapcrew, 8: luateam, 9: funcorp, 10: fashionsquad
                    ulkebyte = self.langue.lower().swapcase()
                    userbyte = self.playerName + " disconnected."
                    self.sendStaffCM(privbyte, ulkebyte, userbyte)

                                    
            if self.privLevel in [9, 10, 11]:
                    privbyte = self.langue.lower().swapcase()
                    userbyte = self.playerName + " disconnected."
                    self.sendStaffCM(2, privbyte, userbyte)
                    self.sendStaffCM(3, privbyte, userbyte)
                    self.sendStaffCM(7, privbyte, userbyte)
                    self.sendStaffCM(9, privbyte, userbyte)
...
##        if password == "":
##            playerName = self.server.checkConnectedAccount("#" + (playerName[0].isdigit() or len(playerName) > 12 or len(playerName) < 3 or "0000" if "+" in playerName else playerName))
        ...
        if self.server.checkConnectedAccount(playerName):
            ...
            if "@" not in playerName:
                ...
                if self.shamanSaves >= 1000000000000000000:
                    self.sendShamanType(self.shamanType, (self.shamanSaves >= 100 and self.hardModeSaves >= 100))
                ...
                if self.privLevel in [5, 6, 7, 8]:
                    privbyte = {5:9, 6:7, 7:3, 8:3}[self.privLevel] # 2: arbitre, 3: modo, 7: mapcrew, 8: luateam, 9: funcorp, 10: fashionsquad
                    ulkebyte = self.langue.lower().swapcase()
                    userbyte = self.playerName + " is now online."
                    self.sendStaffCM(privbyte, ulkebyte, userbyte)
                ...
                reactor.callLater(1200, self.sendAutoMessage)
                

    def sendAutoMessage(self):
        self.sendMessage('<VP>[%s] Discord - https://discord.gg/wTAJQb' %(self.langue))
        reactor.callLater(1200, self.sendAutoMessage)
    ...
    def EventTitleKazan(self, title):
...
##        if not self.room.isTotemEditor and not self.room.isEditor and not self.room.isRacing and not self.room.isBootcamp and not self.room.isSurvivor and not self.room.isVillage and not self.room.isDefilante:
        ...
        if self.room.isFFARace:
            ...
            self.sendMessage("<font color='#E56CA3'>Fastracing modulune hoş geldiniz!</font>\n<font color='#e6b31e'>Lider Tablosu '!lb'</font>")#\n<font color='#e6b31e'>Kırılan kayıtlarınız '!lsrec'</font>") 
        if self.room.isSpeedRace:
            self.sendMessage("[<J>#</J>] Bem-vindo ao módulo <R>Fastracing</R>.\n[<J>#</J>] Para ver o placar, digite '<J>!lb</J>'\n[<J>#</J>] Insira '<J>!listrec </J>' e '<J>!lsrec</J>' para saber mais sobre seus registros quebrados.")
        if self.room.isBigdefilante:
            self.sendMessage("[<J>#</J>] Bem-vindo ao módulo <R>Bigdefilante</R>\n[<J>#</J>] Recorda list, digite '<J>/defrecs</J>'") 
        if self.room.isMeepRace:
            self.sendMessage("[<J>#</J>] Bem-vindo ao módulo <R>Meepracing</R>.\n[<J>#</J>] Pressione a barra de espaço para meep") 
    
        if self.room.isFlyMod:
            self.room.bindKeyBoard(self.playerName, 32, False, self.room.isFly)
            self.sendLangueMessage("", "<N>Press space on the keyboard to use the flights.")
            
        if self.room.isFuncorp:
            self.sendLangueMessage("", "<FC>$FunCorpActive</FC>")

        if self.room.porno:
            self.sendMessage("<ROSE>You want Porn Star?\n<VP>The new porn star of the future <J>%s" %(self.playerName))
...
##    def firstcounts8(self):
##        self.firstCount += 350
##        self.cheeseCount += 350
##        self.sendMessage("<font color='#729169'>[Soldier] Everyone in the room win 350 first.")

    def sendSaintConsumables(self):
        id = 2236
        if not id in self.playerConsumables:
            self.playerConsumables[id] = 5
        else:
            count = self.playerConsumables[id] + 5
            self.playerConsumables[id] = count
            self.sendAnimZeldaInventory(4, id, 5)
    ...
    def sendSaintMessage(self):
        self.sendMessage("<font color='#EA489E'>[Saint Valentine] Happy Valentine's Day ^^</font>")

    def sendSaintTitle(self):
        self.specialTitleList.append(250.1)
        self.sendUnlockedTitle(250, 1)
        self.sendCompleteTitleList()
        self.sendTitleList()

    def sendSaintEvent(self):
        self.sendSaintMessage()
        reactor.callLater(20, self.sendSaintTitle)
        reactor.callLater(40, self.sendSaintBadge)
        reactor.callLater(60, self.sendSaintConsumables)

    #def sendStartSaintEvent(self):
        #self.room.mapCode == 20410
    ...
    def startPlay(self):
       ...
       # if self.room.mapCode == 20001 or self.room.mapCode == 20002:
            #self.sendPacket([8, 30], ByteArray().writeInt(-20).writeUTF("Fish Man").writeShort(335).writeBoolean(True).writeUTF("87;58,6,0,0,0,0,0,0,0").writeShort(1625).writeShort(356).writeShort(11).writeByte(0).writeShort(0).toByteArray())
            #self.sendNPC(1, 4, "Fish Man", 335, "87;58,6,0,0,0,0,0,0,0", 1625, 356, 11, 0)
        if self.room.porno:
            self.room.sendAll(Identifiers.old.send.Music, ["http://62.138.24.129/tb/8/72/ahlayan_kari_remix_10_dakikalik_mp3_59320.mp3"])
        ...
        if self.room.mapCode == 20410:
            #self.sendPacket([8, 30], ByteArray().writeInt(-20).writeUTF("Mark").writeShort(116).writeBoolean(True).writeUTF("122;174_FFB0AF+FFBCBB+FFC3C2,0,0,66_FFC3C2,0,3_D49F9E,0,9_F6A1A0+F1BDA9+FFD0DA,10_FFC9C8+FFD2D2+FF6461+FFC6C5,0,0").writeShort(600).writeShort(359).writeShort(1).writeByte(11).writeShort(0).toByteArray())
            #self.sendPacket([8, 30], ByteArray().writeInt(-20).writeUTF("Gwen").writeShort(116).writeBoolean(True).writeUTF("108;172,20,0,0,40,0,0,0,0,0,0").writeShort(654).writeShort(359).writeShort(1).writeByte(11).writeShort(0).toByteArray())
            self.sendSaintEvent()
    ...
    def playerWin(self, holeType, distance=-1):
        ...
        if timeTaken > 5:
            ...
            if canGo:
                                ...
                                if self.room.mapCode not in (-1, 31, 41, 42, 54, 55, 59, 60, 62, 89, 92, 99, 114, 801):
                                    try:
                                        ...
                                        if timeDB[0] == 0 or timeTaken < timeDB[0]:
                                            ...
                                            for client in self.room.clients.values():
                                                    client.sendMessage("[<J>#</J>] <J>%s</J> <BL>set a new record for this map in <V>%s</V> second." %(self.playerName,t))
                ...
                if self.room.getPlayerCountUnique() >= self.server.needToFirst:
                   if self.room.isRacing or self.room.isSpeedRace or self.room.isMeepRace:
                       self.racingRounds += 1
                       if self.racingRounds >= 5:
                           self.addConsumable(2254, 2)
                           self.racingRounds = 0

                   if self.room.isBootcamp:
                       self.bootcampRounds += 1
                       if self.bootcampRounds == 5:
...
##            self.titleList.extend([440.1, 442.1, 444.1, 445.1, 446.1, 447.1, 448.1, 449.1, 450.1, 451.1, 452.1, 453.1,]) 
##                
        if self.privLevel == 8:
            self.titleList.extend([440.1, 442.1, 444.1, 445.1, 446.1, 447.1, 448.1, 449.1, 450.1, 451.1, 452.1, 453.1,]) 
        if self.privLevel == 9:
        ...
        if self.privLevel in [1, 2, 5, 6, 7, 8, 9, 10, 11]:
            self.titleList.extend([9000.1, 9001.1, 9002.1, 9003.1, 9004.1, 9005.1, 9006.1, 9007.1, 9008.1, 9009.1, 9010.1, 9011.1, 9012.1, 9013.1, 9014.1, 9015.1, 9016.1, 9017.1, 9018.1, 9019.1, 9020.1, 9021.1, 9022.1, 9023.1, 9024.1, 9025.1, 9026.1, 9027.1, 9028.1,9029.1,9030.19,9031.1,9032.1,9033.1,9034.1,9035.1,9036.1,9037.1,9038.1,9039.1,9040.1,9041.1,9042.1,9043.1,9044.1,9045.1,9046.1,9047.1,9048.1,9049.1,9050.1,9051.1,9052.1,9053.1,9054.1,9055.1,9056.1,9057.1,9058.1,9059.1,9060.1])    
    ...
    def sendProfile(self, playerName):
        ...
        if player != None and not player.isGuest:
            packet = ByteArray().writeUTF(player.playerName).writeInt(player.playerID).writeInt(str(player.regDate)[:10]).writeByte({1:1, 2:1, 5:13, 6:11, 7:1, 8:5, 9:10, 10:10, 11:10}[player.privLevel]).writeByte(player.gender).writeUTF(player.tribeName).writeUTF(player.marriage)
    ...
    def sendModInfo(self, mode):
        if self.privLevel >= 3:
            ...
            for mod in self.server.players.values():
                if mod.privLevel >= 3:
                    mod.sendMessage("<%s>• [%s] %s %s." % ("VI " if self.privLevel > 5 else "S", self.langue.upper(), self.playerName, "just connected" if bool(mode) else "has disconnected"))
                    if mod.playerName != self.playerName:
                        self.sendMessage("<%s>• [%s] %s : %s" % ("VI" if mod.privLevel > 5 else "S", mod.langue.upper(), mod.playerName, mod.roomName))
        else:
            pass

##    def sendLordInfo(self, mode):
##        if self.playerName in ["Slyvanas", "Batu", "Batu"]:
##            mod = self.server.players.values()
##            for mod in self.server.players.values():
##                if mod.privLevel >= 1:
##                    mod.sendMessage("<%s>%s %s !" % ("font color ='#6C0F10'" if self.privLevel > 10 else "S", self.playerName, "Lord is back" if bool(mode) else ""))
##        else:
##            pass
        ...
        if mode != 18:
            ...
            for checkRoom in self.server.rooms.values():
                if {1:checkRoom.isNormRoom, 3:checkRoom.isVanilla or checkRoom.porno, 8:checkRoom.isSurvivor or checkRoom.isOldSurvivor, 9:checkRoom.isRacing or checkRoom.isSpeedRace or checkRoom.isMeepRace,  11:checkRoom.isMusic, 2:checkRoom.isBootcamp, 10:checkRoom.isDefilante or checkRoom.isBigdefilante, 18:0, 16:checkRoom.isVillage}[mode] and checkRoom.community == self.langue.lower():
                    roomsCount += 1
                    packet.writeByte(0).writeByte(self.langueID).writeUTF(checkRoom.roomName).writeShort(checkRoom.getPlayerCount()).writeUnsignedByte(checkRoom.maxPlayers).writeBoolean(False)
...
class Server(protocol.ServerFactory):
    ...
    def __init__(self):
        ...
        # Files
        self.parseSWF = self.parseFile("./extra/jsons/infoSWF.json")
        self.captchaList = self.parseFile("./extra/jsons/captchas.json")
        self.promotions = self.parseFile("./extra/jsons/promotions.json")
        self.serverList = self.parseFile("./extra/jsons/serverList.json")
        self.menu = self.parseFile("./extra/jsons/menu.json")
        self.npcs = self.parseFile("./extra/jsons/npcs.json")
        ...
        for player in self.players.values():
            if str(type(minLevel)) == "<type 'int'>" and player.privLevel >= minLevel:
                if ModoPwet:
                    ...
                    player.sendMessage(message, tab)
            elif minLevel == "admin" and player.playerName in ["Slyvanas"]:
                player.sendMessage(message, tab)
...
class Room:
    def __init__(self, server, name):
        ...
        elif roomNameCheck.startswith("#pornhub"):
            self.porno = True
            self.isVanilla = True
        ...
        if self.initVotingMode:
                ...
                if rs[0] > 0:
                    ...
                    for player in self.clients.values():
                        ...
                        for player in self.clients.values():
                            ...
                            if player.langueID == 3:
                                player.sendMessage("[<J>#</J>] <R>(@%s)</R> <N>não há registro quebrado no mapa." %(self.mapCode))
                            if player.langueID == 6:
                                player.sendMessage("<R>(@%s)</R> <bl>haritada kırılan bir rekor yok</font>" %(self.mapCode))
            ...
            if self.isBigdefilante:
                ...
                if rs[0] > 0:
                    ...
                    for player in self.clients.values():
                        player.sendMessage("[<J>#</J>] </font> <N>(<R>@"+str(self.mapCode)+"</R>) não há record quebrados neste mapa.</N>")
    ...
    def sendAllChat(self, playerCode, playerName, message, langueID, isOnly):
        ...
        if not isOnly:
            ...
            client = self.clients.get(playerName)
            if client != None:
                client.sendPacket(Identifiers.send.Chat_Message, p.toByteArray())
            self.server.sendStaffMessage(7, "[<V>SPAM</V>][<V>" + client.roomName + "</V>][<T>" + client.playerName + "</T>] sent a link in the message: [<J>" + str(message) + "</J>].")
    ...
    def checkMapXML(self):
        if int(self.currentMap) in self.server.vanillaMaps:
            ...
            self.mapName = "_Village" if self.mapCode == 801 else "TrexlyMice"
...
if __name__ == "__main__":
    ...
    Database = MySQLdb.connect("localhost","root","","server")
