...
class ByteArray:
    ...
    def writeInt(self, value):
        self.write(pack("!I", long(value) & 0xFFFFFFFF))
        return self
...
class DailyQuest:
        ...
        if self.createAccount:
            ...
            while ID < 3:
                if int(self.client.dailyQuest[ID]) == 0:
                    ...
                    if mission[0] == int(self.client.dailyQuest[0]) or int(self.client.dailyQuest[1]) or int(self.client.dailyQuest[2]):
                        mission = self.randomMission()
    ...
    def updateDailyQuest(self, alterDB = False):
        ...
        if rs:
            ...
            self.client.dailyQuest = map(str, filter(None, rs[0].split(","))) if rs[0] != "" else [0, 0, 0, 1]
    ...
    def sendDailyQuest(self):
        ...
        self.client.sendPacket([144, 3], p.toByteArray())
