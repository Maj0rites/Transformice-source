...
##            if self.client.cheeseCount > 3:
            ...
            if self.client.isGuest:
                ...
                packet = ByteArray().writeInt(tribulleID)
                if playerName.startswith("*") or not self.server.players.has_key(playerName):
                    can = False
                    packet.writeByte(12)
                    packet.writeShort(0)
...
##                 self.client.sendMessage("<ROSE>You need 3 cheeses to speak.")            
        ...
        def createTribe(self, readPacket):
            ...
            if len(tribeName) > 50:
                self.client.sendMessage("Max 50 Chararacters")
                return
            if not bool(BeautifulSoup(tribeName, "html.parser").find()):
                ...
                if not self.checkExistingTribe(tribeName):
                    if self.client.shopCheeses >= 500:
                        ...
                        self.client.tribeMessage = "Welcome ^-^"
                        ...
                        self.sendPacket(85, ByteArray().writeInt(self.tribulleID).toByteArray())
            else:
        ...
        def checkFriend(self, playerName, playerNameToCheck):
            checkList = self.server.players[playerName].friendsList if self.server.players.has_key(playerName) else self.getUserFriends(playerName)
        ...
        def getInGenderMarriage(self, playerName):
            if self.server.players.has_key(playerName):
                player = self.server.players.get(playerName)
