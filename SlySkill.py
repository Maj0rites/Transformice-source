...
class Skills:
    ...
    def buySkill(self, skill):
        if self.client.shamanLevel - 1 > len(self.client.playerSkills):
            if self.client.playerSkills.has_key(skill):
    ...
    def getTimeSkill(self):
        if self.client.playerSkills.has_key(0):
            self.client.room.addTime += self.client.playerSkills[0] * 5

    def getkills(self):
        if self.client.isShaman:
            if self.client.playerSkills.has_key(4) and not self.client.room.isDoubleMap:
            ...
            for skill in [5, 8, 9, 11, 12, 26, 28, 29, 31, 41, 46, 48, 51, 52, 53, 60, 62, 65, 66, 67, 69, 71, 74, 80, 81, 83, 85, 88, 90, 93]:
                if self.client.playerSkills.has_key(skill) and not (self.client.room.isSurvivor and skill == 81):
            ...
            if self.client.playerSkills.has_key(91):
    ...
    def parseEmoteSkill(self, emote):
        count = 0
        if emote == 0 and self.client.playerSkills.has_key(3):
            for player in self.client.room.clients.values():
        ...
        elif emote == 4 and self.client.playerSkills.has_key(61):
            for player in self.client.room.clients.values():
        ...
        elif emote == 8 and self.client.playerSkills.has_key(25):
            for player in self.client.room.clients.values():
